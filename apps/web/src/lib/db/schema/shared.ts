...
// Shared user table schema (mirrors server auth.ts)
...
// Chat-specific schemas for local storage
...
export const syncConfig = sqliteTable("sync_config", {
  ...
  syncInterval: integer("sync_interval").default(30000), // 30 seconds
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull(),
});
