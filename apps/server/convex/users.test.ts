...
describe("users.ensure", () => {
	...
	test("should return existing user if already exists", async () => {
		...
		const result1 = await t.mutation(api.users.ensure, {
			externalId: "user_existing",
			email: "old@example.com",
			name: "Old Name",
		});

		const result2 = await t.mutation(api.users.ensure, {
			externalId: "user_existing",
			email: "old@example.com",
			name: "Old Name",
		});
	...
	test("should enforce rate limit on user creation", async () => {
		...
		// Create many users quickly to hit rate limit
		// Run them sequentially until one fails to avoid breaking test state
		let rateLimitHit = false;
		for (let i = 0; i < 150; i++) {
			try {
				await t.mutation(api.users.ensure, {
					externalId: `user_rate_limit_${i}`,
				});
			} catch (error: any) {
				if (error.message && error.message.includes("Too many authentication attempts")) {
					rateLimitHit = true;
					break;
				}
				throw error;
	...
	test("should handle optional fields correctly", async () => {
		...
		const result3 = await t.mutation(api.users.ensure, {
...
describe("users.getById", () => {
	...
	test("should return null for invalid ID", async () => {
		...
		const user = await t.query(api.users.getById, { userId: tempId });
...
describe("users.getOpenRouterKey", () => {
	...
	test("should return null for non-existent user", async () => {
		...
		const key = await t.query(api.users.getOpenRouterKey, { userId: tempId });
...
describe("users.removeOpenRouterKey", () => {
	...
	test("should remove OpenRouter key", async () => {
		...
		const result = await t.mutation(api.users.removeOpenRouterKey, {
			userId,
		});
	...
	test("should enforce rate limit on API key removals", async () => {
		...
		for (let i = 0; i < 10; i++) {
			try {
				await t.mutation(api.users.removeOpenRouterKey, {
					userId,
				});
			} catch (error: any) {
