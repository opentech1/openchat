...
import { resolveAllowedOrigins, validateRequestOrigin } from "./request-origin";
...
export type ChatHandlerOptions = {
  ...
  resolveModel?: (input: { request: Request; payload: any }) => Promise<{ provider: ReturnType<typeof createOpenRouter>; modelId: string }>;
...
export function createChatHandler(options: ChatHandlerOptions) {
  ...
  const corsOrigin = options.corsOrigin ?? process.env.CORS_ORIGIN ?? process.env.NEXT_PUBLIC_APP_URL;
  const allowedOrigins = resolveAllowedOrigins(corsOrigin);

  const persistMessageImpl = options.persistMessage;
  ...
  return async function handler(request: Request): Promise<Response> {
    ...
    if (!originResult.ok) return new Response("Invalid request origin", { status: 403 });
    ...
    catch { return new Response("Invalid JSON payload", { status: 400, headers: buildCorsHeaders(request, allowOrigin) }); }
    ...
    try {
      if (typeof options.resolveModel === "function") {
        ...
        if (!apiKeyFromPayload) throw new Error("Missing apiKey in request payload");
        if (!modelIdFromPayload) throw new Error("Missing modelId in request payload");
      ...
      if (messageText.includes("Missing apiKey")) { status = 400; responseMessage = "Missing apiKey"; }
      else if (messageText.includes("Missing modelId")) { status = 400; responseMessage = "Missing modelId"; }
      return new Response(responseMessage, { status, headers });
    }

    const headerUser = request.headers.get("x-user-id");
    const distinctId = options.distinctId ?? (headerUser ? headerUser.trim() : null);
    const chatId = typeof payload?.chatId === "string" && payload.chatId.trim().length > 0 ? payload.chatId.trim() : null;
    if (!chatId) return new Response("Missing chatId", { status: 400, headers: buildCorsHeaders(request, allowOrigin) });

    const rawMessages: AnyUIMessage[] = Array.isArray(payload?.messages) ? (payload.messages as AnyUIMessage[]) : [];
    if (rawMessages.length === 0) return new Response("Missing chat messages", { status: 400, headers: buildCorsHeaders(request, allowOrigin) });

    const safeMessages = rawMessages.map(clampUserText);
    const userMessageIndex = [...rawMessages].reverse().findIndex((msg) => msg.role === "user");
    if (userMessageIndex === -1) return new Response("Missing user message", { status: 400, headers: buildCorsHeaders(request, allowOrigin) });
    ...
    const userMessageId = typeof rawUserMessage.id === "string" && rawUserMessage.id.length > 0
      ? rawUserMessage.id : (crypto.randomUUID?.() ?? `user-${Date.now()}`);
    ...
    if ((safeUserMessage as any).id !== userMessageId) { (safeUserMessage as any).id = userMessageId; }

    const assistantMessageId = typeof payload?.assistantMessageId === "string" && payload.assistantMessageId.length > 0
      ? payload.assistantMessageId : (crypto.randomUUID?.() ?? `assistant-${Date.now()}`);
    const assistantCreatedAtIso = new Date().toISOString();

    try {
      const userResult = await persistMessageImpl({
        chatId, messageId: userMessageId, role: "user", content: userContent, createdAt: userCreatedAtIso, status: "completed",
      });
      if (!userResult.ok) throw new Error("user streamUpsert rejected");
    } catch (error) {
      return new Response("Failed to persist message", { status: 502, headers: buildCorsHeaders(request, allowOrigin) });
    }

    try {
      const assistantBootstrap = await persistMessageImpl({
        chatId, messageId: assistantMessageId, role: "assistant", content: "", createdAt: assistantCreatedAtIso, status: "streaming",
      });
      if (!assistantBootstrap.ok) throw new Error("assistant streamUpsert rejected");
    } catch (error) {
      return new Response("Failed to persist assistant", { status: 502, headers: buildCorsHeaders(request, allowOrigin) });
    }
    ...
    const persistAssistant = async (status: "streaming" | "completed", force = false) => {
      if (!force && status === "streaming" && assistantText.length === lastPersistedLength) return;
      lastPersistedLength = assistantText.length;
      const response = await persistMessageImpl({ chatId, messageId: assistantMessageId, role: "assistant", content: assistantText, createdAt: assistantCreatedAtIso, status });
      if (!response.ok) throw new Error("assistant streamUpsert rejected");
    };

    const scheduleStreamFlush = () => {
      if (flushTimeout) return pendingFlush ?? Promise.resolve();
      const promise = new Promise<void>((resolve) => { pendingResolve = resolve; });
      pendingFlush = promise;
      flushTimeout = setTimeout(async () => {
        flushTimeout = null;
        try { await persistAssistant("streaming"); }
        catch (error) { if (!persistenceError) persistenceError = error instanceof Error ? error : new Error("Failed to persist assistant chunk"); }
        finally { pendingResolve?.(); pendingResolve = null; pendingFlush = null; }
    ...
    const finalize = async () => {
      ...
      try { await persistAssistant("completed", true); }
      catch (error) { if (!persistenceError) persistenceError = error instanceof Error ? error : new Error("Failed to persist assistant completion"); }
    ...
    try {
      ...
      const result = await streamTextImpl({
        ...
        onChunk: async ({ chunk }) => { if (chunk.type === "text-delta" && chunk.text.length > 0) { assistantText += chunk.text; scheduleStreamFlush(); } },
        onFinish: async () => { streamStatus = "completed"; await finalize(); },
        onAbort: async () => { streamStatus = "aborted"; await finalize(); },
        onError: async () => { streamStatus = "error"; await finalize(); },
      });
      capturePosthogEvent("chat_message_stream", options.distinctId ?? null, {
        chatId, modelId: config.modelId, userMessageId, assistantMessageId, characters: assistantText.length, durationMs: Date.now() - startedAt, status: streamStatus,
      });

      const aiResponse = result.toUIMessageStreamResponse({ generateMessageId: () => assistantMessageId });
      ...
      capturePosthogEvent("chat_message_stream", options.distinctId ?? null, {
        chatId, modelId: config.modelId, userMessageId, assistantMessageId, characters: assistantText.length, status: "error",
      });
      return new Response("Upstream error", { status: 502, headers: buildCorsHeaders(request, allowOrigin) });
    }
  };
}
